...
def evaluate_model(config):
    ...
    X_test = X_test[:, -1, :]  # Используем только последний временной срез
    ...
    model.load_state_dict(torch.load("models/best_model_mlp.pth"))
    model.eval()
    ...
    with open("results/metrics_mlp.txt", "w") as f:
    ...
    plt.title("MLP: Sample Predictions vs Actual")
    plt.legend()
    plt.savefig("results/predictions_mlp.png")
    plt.close()

    plt.figure(figsize=(8, 4))
    plt.hist(y_probs, bins=50, color='skyblue', edgecolor='black')
    plt.title("MLP: Distribution of Predicted Probabilities")
    plt.xlabel("Predicted Probability")
    plt.ylabel("Frequency")
    plt.savefig("results/probability_distribution_mlp.png")
    plt.close()
